...
subroutine modal_aero_sw(list_idx, state, pbuf, nnite, idxnite, is_cmip6_volc, ext_cmip6_sw, trop_level,  &
   ...
   real(r8), intent(out) :: tauxar(pcols,pver,nswbands) ! layer extinction optical depth
   real(r8), intent(out) :: wa(pcols,pver,nswbands)     ! layer single-scatter albedo
   real(r8), intent(out) :: ga(pcols,pver,nswbands)     ! asymmetry factor
   real(r8), intent(out) :: fa(pcols,pver,nswbands)     ! forward scattered fraction
