...
subroutine aer_rad_props_sw(list_idx, state, pbuf,  nnite, idxnite, is_cmip6_volc, &
   ...
   real(r8), intent(out) :: tau    (pcols,pver,nswbands) ! aerosol extinction optical depth
   real(r8), intent(out) :: tau_w  (pcols,pver,nswbands) ! aerosol single scattering albedo * tau
   real(r8), intent(out) :: tau_w_g(pcols,pver,nswbands) ! aerosol assymetry parameter * tau * w
   real(r8), intent(out) :: tau_w_f(pcols,pver,nswbands) ! aerosol forward scattered fraction * tau * w
...
subroutine volcanic_cmip_sw (state, pbuf, trop_level, ext_cmip6_sw, tau, tau_w, tau_w_g, tau_w_f)
  ...
  !Intent-inout
  real(r8), intent(inout) :: tau    (pcols,pver,nswbands) ! aerosol extinction optical depth
  real(r8), intent(inout) :: tau_w  (pcols,pver,nswbands) ! aerosol single scattering albedo * tau
  real(r8), intent(inout) :: tau_w_g(pcols,pver,nswbands) ! aerosol assymetry parameter * tau * w
  real(r8), intent(inout) :: tau_w_f(pcols,pver,nswbands) ! aerosol forward scattered fraction * tau * w 
