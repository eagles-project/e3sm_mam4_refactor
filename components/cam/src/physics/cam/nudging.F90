...
!!==>SZ ADD
  public:: Nudge_Zconstrain
  public:: Nudge_Z3thres
!!==>SZ END
...
!!==> JS ADD
  ...
  !!==>SZ ADD
  logical            :: Nudge_Zconstrain
  real(r8)           :: Nudge_Z3thres
  !!==>SZ END
...
!!==> JS END
!!==> SZ ADD
                         Nudge_Zconstrain, Nudge_Z3thres
!!==> SZ END
...
!!==> SZ ADD
   call mpibcast(Nudge_Zconstrain,1,mpilog,0,mpicom)
   call mpibcast(Nudge_Z3thres,1,mpir8,0,mpicom)
!!==> SZ END
...
!!==> SZ ADD
     write(iulog,*) 'NUDGING: Nudge_Zconstrain    =',Nudge_Zconstrain
     write(iulog,*) 'NUDGING: Nudge_Z3thres       =',Nudge_Z3thres
!!==> SZ END
     write(iulog,*) ' '
     write(iulog,*) ' '
...
!==> SZ ADD
!use surface geoptential to rule out the nudging tendencies that is 
!potentially contaminated by the inconsistencies of topography in 
!reanalysis and E3SM 
     if (Nudge_Zconstrain) then
      do i=1,ncol
        if(phys_state%phis(i).ge.Nudge_Z3thres) then
          do k=1,pver
           phys_tend%u(i,k)      = 0._r8
           phys_tend%v(i,k)      = 0._r8
           phys_tend%s(i,k)      = 0._r8
           phys_tend%q(i,k,indw) = 0._r8
          enddo
        end if 
      end do
     end if
