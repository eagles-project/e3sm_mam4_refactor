module clubb_intr
  ...
  use pbl_utils,     only: calc_ustar, calc_obklen  ! These are CAM's util functions, not CLUBB's
  use perf_mod,      only: t_startf, t_stopf
  use mpishorthand
  use cam_history_support, only: fillvalue

  use clubb_precision,     only: core_rknd   !Precision used for internal calculations in CLUBB
...
!-------------------------------------------------------------------------------
    ...
    real(kind=core_rknd), dimension(nparams)  :: clubb_params    ! These adjustable CLUBB parameters (C1, C2 ...)
    ...
    !  (Notes by Hui Wan, 2018-11-17:
    !   In the next subroutine call, because the second argument is "",
    !   the model will NOT read namelist files. But rather, 
    !   the values already read in during clubb_param_readnl
    !   are used to update CLUBB's internal tunable parameters.
    !   These parameters, originally saved in scalar variables,
    !   are packed into the array "clubb_params". 
    !   In subr. clubb_tend_cam, "setup_parameters" will be called 
    !   every time step. In there, clubb_params will be unpacked.
!$OMP PARALLEL
...
!$OMP PARALLEL
    ...
    call setup_clubb_core     &
         ( pverp, real(theta0,  kind = core_rknd), ts_nudge, &        ! In
...
!-------------------------------------------------------------------------------
   ...
   real( kind = core_rknd ) :: dtime_core_rknd  ! CLUBB time step [s]   
   ...
   real(core_rknd) :: rho_tmp(pcols,pverp)      ! Midpoint density in CAM                       [kg/m^3]
 ...
 !  Initialize physics tendency arrays, copy the state to state1 array to use in this routine
   ...
   do i=1,ncol   ! loop over columns
      ...
      do k=1,pver
         ...
         rho_tmp(i,k+1)       = rho_ds_zt(k+1)                                       ! rho on thermo 
      ...
      dum_core_rknd = 1._core_rknd/rho_ds_zm(1) 
      wpthlp_sfc = real( cam_in%shf(i)/cpair , kind=core_rknd )*dum_core_rknd  ! Sensible heat flux
      wprtp_sfc  = real( cam_in%cflx(i,1)    , kind=core_rknd )*dum_core_rknd  ! Latent heat flux
      upwp_sfc   = real( cam_in%wsx(i)       , kind=core_rknd )*dum_core_rknd  ! Surface meridional momentum flux
      vpwp_sfc   = real( cam_in%wsy(i)       , kind=core_rknd )*dum_core_rknd  ! Surface zonal momentum flux  
      ...
      ! --------------------------------------------------------- !  
      dtime_core_rknd = real( dtime ,kind=core_rknd )
      ...
      do t=1,nadv    ! do needed number of "sub" timesteps for each CAM step
         ...
         call advance_clubb_core &
            ( l_implemented, dtime_core_rknd, fcor, sfc_elevation, hydromet_dim, &
         ...
         if (do_rainturb) then
            ...
            call update_xp2_mc(pverp, dtime_core_rknd, cloud_frac_out, &
            ...
            clubb_rnevap_effic_core_rknd = real( clubb_rnevap_effic ,kind=core_rknd )

            rtp2_in  = rtp2_in + clubb_rnevap_effic_core_rknd * dum_core_rknd * rtp2_mc_out * dtime_core_rknd
            thlp2_in = thlp2_in + clubb_rnevap_effic_core_rknd * dum_core_rknd * thlp2_mc_out * dtime_core_rknd
            if (.not. clubb_do_deep) then
               wprtp_in = wprtp_in + clubb_rnevap_effic_core_rknd * dum_core_rknd * wprtp_mc_out * dtime_core_rknd
               wpthlp_in = wpthlp_in + clubb_rnevap_effic_core_rknd * dum_core_rknd * wpthlp_mc_out * dtime_core_rknd
            endif
!                     rtpthlp_in = rtpthlp_in + rtpthlp_mc_out * dtime_core_rknd
         endif     
         ...
         if (do_cldcool) then
            ...
            thlp2_in = thlp2_in + thlp2_rad_out * dtime_core_rknd
