...
!>songxl 2011-09-20----------------------------
    ...
    !Shixuan Zhang & HuiWan (2020/07): end of specificing the required variables for tendnecy dribbling 

    !HuiWan (2021/02): added for new radiation coupling test ++ 
    integer :: radheat_cpl_opt
    real(r8):: zqrl(pcols,pver) ! longwave heating
    real(r8):: zqrs(pcols,pver) ! shortwave heating
...
!!== KZ_WATCON 
       ...
       if ( l_dribble_tend_into_macmic_loop ) then
         ...
         ! Save relevant prognostic variable to pbuf for the next cycle's of dribbling 
...
subroutine add_fld_default_calls()
  ...
  character(len=20), parameter :: vlist(29) = (/     'topphysbc           '                       ,&
  ...
  do ihist = 1 , nvars_prtrb_hist
     do iv = 1, ntot   
        ...
        if(trim(adjustl(vlist(iv))).eq."clubb_ice1" .or. &
           ...
           trim(adjustl(vlist(iv))).eq."cldwat_mic") then

          do imacmic = 1,cld_macmic_num_steps
            write(tsubname,"(A3,I2.2)")"sub",imacmic
            varname  = trim(adjustl(hist_vars(ihist)))//'_'//trim(adjustl(vlist(iv)))//'_'//trim(adjustl(tsubname))
