...
!$OMP PARALLEL DO PRIVATE (C, NCOL, phys_buffer_chunk)
    ...
    if (begchunk.le.endchunk) then ! AaronDonahue added for parallel-split
...
if (l_tracer_aero) then
    ...
    if (chem_is_active().and.ncol.gt.0) then  ! Changed by AaronDonahue to include ncol
...
if (l_gw_drag) then
    ...
    if (ncol.gt.0) then ! AaronDonahue 
