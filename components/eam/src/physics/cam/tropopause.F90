...
!BJG  subroutine tropopause_climate(pstate, tropLev, tropP, tropT, tropZ)
...
!BJG    type(physics_state), intent(in)    :: pstate 
...
!BJG    lchnk = pstate%lchnk
!BJG    ncol  = pstate%ncol
...
!BJG            tropT(icol) = tropopause_interpolateT(pstate, icol, tropLev(icol), tP)
          end if

          if (present(tropZ)) then
            tropZ(icol) = tropopause_interpolateZ(pint,pmid,zm,zi,icol,tropLev(icol),tP)
!BJG            tropZ(icol) = tropopause_interpolateZ(pstate, icol, tropLev(icol), tP)
          end if
        end if
      end do
    end if        
  ...
  !-----------------------------------------------------------------------
  !-----------------------------------------------------------------------
!BJG  subroutine tropopause_hybridstobie(pstate, tropLev, tropP, tropT, tropZ)
...
!BJG    type(physics_state), intent(in)     :: pstate 
  ...
  !
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!BJG  subroutine twmo(t, p, plimu, pliml, gam, trp)
...
!BJG    level = size(t)
!BJG    pmk= .5_r8 * (p(level-1)**cnst_kap+p(level)**cnst_kap)
    pm = pmk**(1/cnst_kap)
...
!BJG    a = (t(level-1)-t(level))/(p(level-1)**cnst_kap-p(level)**cnst_kap)
!BJG    b = t(level)-(a*p(level)**cnst_kap)
!BJG    tm = a * pmk + b               
!BJG    dtdp = a * cnst_kap * (pm**cnst_ka1)
...
!BJG      pmk= .5_r8 * (p(j-1)**cnst_kap+p(j)**cnst_kap)
      pm = pmk**(1/cnst_kap)               
      a1 = (temp1d(kk-1)-temp1d(kk))/(pmid1d(kk-1)**cnst_kap-pmid1d(kk)**cnst_kap)
      b1 = temp1d(kk)-(a1*pmid1d(kk)**cnst_kap)
      tm = a1 * pmk + b1
      dtdp = a1 * cnst_kap * (pm**cnst_ka1)
!BJG      a = (t(j-1)-t(j))/(p(j-1)**cnst_kap-p(j)**cnst_kap)
!BJG      b = t(j)-(a*p(j)**cnst_kap)
!BJG      tm = a * pmk + b               
!BJG      dtdp = a * cnst_kap * (pm**cnst_ka1)
      dtdz = cnst_faktor*dtdp*pm/tm
      ! dt/dz valid?
...
!BJG        pmk2 = .5_r8 * (p(jj-1)**cnst_kap+p(jj)**cnst_kap) ! p mean ^kappa
...
!BJG        a2 = (t(jj-1)-t(jj))                     ! a
!BJG        a2 = a2/(p(jj-1)**cnst_kap-p(jj)**cnst_kap)
!BJG        b2 = t(jj)-(a2*p(jj)**cnst_kap)          ! b
...
!BJG  subroutine tropopause_twmo(pstate, tropLev, tropP, tropT, tropZ)
...
!BJG        call twmo(pstate%t(i, :), pstate%pmid(i, :), plimu, pliml, gam, tP)
...
!BJG            tropZ(i) = tropopause_interpolateZ(pstate, i, tropLev(i), tP)
          end if
        end if
      end if
    end do
...
!BJG  subroutine tropopause_cpp(pstate, tropLev, tropP, tropT, tropZ)
      ...
      if (tropLev(icol) == NOTFOUND) then
        ...
        kloop: do kk = pver-1, 2, -1
          ...
          if (zm(icol, kk) < ztrop_low) then
            firstk = kk
            cycle kloop
          else if (zm(icol, kk) > ztrop_high) then
            lastk = kk
            exit kloop
          end if
        ...
        if ((tropLev(icol) >= (firstk-1)) .or. (tropLev(icol) <= (lastk+1))) then
          ...
          if (present(tropP) .or. present(tropZ) .or. present(tropT)) then
            ...
            if ((tZ >= x0) .or. (tZ <= x2)) then
              ...
              ! Return the optional outputs
              if (present(tropP)) then
!BJG                tropP(i) = tropopause_interpolateP(pstate, i, tropLev(i), tZ)
...
!BJG  subroutine tropopause_find(pstate, tropLev, tropP, tropT, tropZ, primary, backup)
...
!BJG      call tropopause_findUsing(pstate, primAlg, tropLev, tropP, tropT, tropZ)
    end if
...
!BJG      call tropopause_findUsing(pstate, backAlg, tropLev, tropP, tropT, tropZ)
    end if
...
!BJG  subroutine tropopause_findUsing(pstate, algorithm, tropLev, tropP, tropT, tropZ)
...
!BJG        call tropopause_climate(pstate, tropLev, tropP, tropT, tropZ)
...
!BJG        call tropopause_hybridstobie(pstate, tropLev, tropP, tropT, tropZ)

      case(TROP_ALG_TWMO)
        call tropopause_twmo(ncol,pmid,pint,temp,zm,zi,tropLev,tropP,tropT,tropZ)
!BJG        call tropopause_twmo(pstate, tropLev, tropP, tropT, tropZ)
...
!BJG        call tropopause_cpp(pstate, tropLev, tropP, tropT, tropZ)
...
!BJG  function tropopause_interpolateP(pstate, icol, tropLev, tropZ)
...
!BJG        dlogPdZ = (log(pstate%pmid(icol, tropLev)) - log(pstate%pmid(icol, tropLev - 1))) / &
!BJG          (pstate%zm(icol, tropLev) - pstate%zm(icol, tropLev - 1)) 
!BJG        tropP = pstate%pmid(icol, tropLev) + exp((tropZ - pstate%zm(icol, tropLev)) * dlogPdZ)
      end if
    else
      ...
      if (tropLev < pver) then
        ...
        tropP = pmid(icol, tropLev) + exp((tropZ - zm(icol, tropLev)) * dlogPdZ)
!BJG        dlogPdZ =  (log(pstate%pmid(icol, tropLev + 1)) - log(pstate%pmid(icol, tropLev))) / &
!BJG          (pstate%zm(icol, tropLev + 1) - pstate%zm(icol, tropLev))
!BJG        tropP = pstate%pmid(icol, tropLev) + exp((tropZ - pstate%zm(icol, tropLev)) * dlogPdZ)
      end if
    end if
...
!BJG  function tropopause_interpolateT(pstate, icol, tropLev, tropP)
...
!BJG        dTdlogP = (pstate%t(icol, tropLev) - pstate%t(icol, tropLev - 1)) / & 
!BJG          (log(pstate%pmid(icol, tropLev)) - log(pstate%pmid(icol, tropLev - 1)))
!BJG        tropT = pstate%t(icol, tropLev) + (log(tropP) - log(pstate%pmid(icol, tropLev))) * dTdlogP
      end if
    else
      ...
      if (tropLev < pver) then
        ...
        tropT = temp(icol, tropLev) + (log(tropP) - log(pmid(icol, tropLev))) * dTdlogP
!BJG        dTdlogP = (pstate%t(icol, tropLev + 1) - pstate%t(icol, tropLev)) / &
!BJG          (log(pstate%pmid(icol, tropLev + 1)) - log(pstate%pmid(icol, tropLev)))
!BJG        tropT = pstate%t(icol, tropLev) + (log(tropP) - log(pstate%pmid(icol, tropLev))) * dTdlogP
      end if
    end if
...
!BJG  function tropopause_interpolateZ(pstate, icol, tropLev, tropP)
...
!BJG      dZdlogP = (pstate%zm(icol, tropLev) - pstate%zi(icol, tropLev)) / &
!BJG        (log(pstate%pmid(icol, tropLev)) - log(pstate%pint(icol, tropLev)))
!BJG      tropZ = pstate%zm(icol, tropLev) + (log(tropP) - log(pstate%pmid(icol, tropLev))) * dZdlogP
    else
      ...
      tropZ = zm(icol, tropLev) + (log(tropP) - log(pmid(icol, tropLev))) * dZdlogP
!BJG      dZdlogP = (pstate%zm(icol, tropLev) - pstate%zi(icol, tropLev+1)) / &
!BJG        (log(pstate%pmid(icol, tropLev)) - log(pstate%pint(icol, tropLev+1)))
!BJG      tropZ = pstate%zm(icol, tropLev) + (log(tropP) - log(pstate%pmid(icol, tropLev))) * dZdlogP
    end if
