...
!!  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('tropopause_interpolateP', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'tropopause_interpolateP',y_nstep, yaml%lev_print)
        ...
        call write_var(unit_input,unit_output,'zm',zm)
        call write_var(unit_input,unit_output,'zm_p1',zm_p1)
        call write_var(unit_input,unit_output,'zm_m1',zm_m1)
        call write_var(unit_input,unit_output,'tropLev',tropLev)
        call write_var(unit_input,unit_output,'tropZ',tropZ)
