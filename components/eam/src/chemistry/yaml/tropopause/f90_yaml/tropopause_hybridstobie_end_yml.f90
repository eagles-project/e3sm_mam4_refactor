  ...
  if(yaml%flag_print) then ! if this column exists in lchnk
     ...
     ! below are diagnostic outputs sent to outfld

        call write_var(unit_output,'trop_output',trop_output(yaml%col_print,:))
        call write_var(unit_output,'trop_linoz_output',trop_linoz_output(yaml%col_print,:))
        call write_var(unit_output,'trop_trop_output',trop_trop_output(yaml%col_print,:))
