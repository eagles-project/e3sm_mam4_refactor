...
!  print output from timestep nstep_print_lo to yaml%nstep_print
 ...
 ! should be invoked by tropopause_twmo
...
!!  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
  ...
  !  call is within external icol loop and is independent of k, so use below condition
  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('tropopause_interpolateT', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'tropopause_interpolateT',y_nstep, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'pmid',pmid)
        call write_var(unit_input,unit_output,'pmid_p1',pmid_p1)
        call write_var(unit_input,unit_output,'pmid_m1',pmid_m1)
        call write_var(unit_input,unit_output,'temp',temp)
        call write_var(unit_input,unit_output,'temp_p1',temp_p1)
        call write_var(unit_input,unit_output,'temp_m1',temp_m1)
        call write_var(unit_input,unit_output,'tropLev',tropLev)
        call write_var(unit_input,unit_output,'tropP',tropP)
