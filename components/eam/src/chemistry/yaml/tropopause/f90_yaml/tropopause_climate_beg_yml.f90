 ...
 !  print output from timestep nstep_print_lo to yaml%nstep_print
  ...
  real(r8)      :: calday_yaml                   ! day of year including fraction
...
!  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
  ...
  if(yaml%col_print >0) then
     write(102,*) 'yaml%col_print, y_nstep:  ',yaml%col_print, y_nstep
  endif
  !-----------------------------------------------------------------------------------------
  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('tropopause_climate', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'tropopause_climate',y_nstep, yaml%lev_print)

        ! add code for writing data here
        ! below are explicit input arguments to tropopause_climate
        ...
        ! below are optional input arguments to tropopause_climate

        if(present(tropP)) then
           call write_var(unit_input,unit_output,'tropP',tropP(yaml%col_print))
        endif
        if(present(tropT)) then
           call write_var(unit_input,unit_output,'tropT',tropT(yaml%col_print))
        endif
        if(present(tropZ)) then
           call write_var(unit_input,unit_output,'tropZ',tropZ(yaml%col_print))
        endif

        ! below are external module inputs to tropopause_climate

        call write_var(unit_input,unit_output,'pcols',pcols)
        call write_var(unit_input,unit_output,'pver',pver)

        ! below are internal module inputs to tropopause_climate

        call write_var(unit_input,unit_output,'NOTFOUND',NOTFOUND)
        call write_var(unit_input,unit_output,'first_month',first_month)
        call write_var(unit_input,unit_output,'months_in_year',months_in_year)
        call write_var(unit_input,unit_output,'days_in_year_real',days_in_year_real)
        call write_var(unit_input,unit_output,'tropp_p_loc',tropp_p_loc(yaml%col_print,lchnk,:))

!       Subroutine tropopause_climate uses as input the output of get_curr_calday(), which is internally
!       assigned to the local variable 'calday'.  So repeat the procedure here, and write this field to the data files

        calday_yaml = get_curr_calday()
        call write_var(unit_input,unit_output,'calday_yaml',calday_yaml)
