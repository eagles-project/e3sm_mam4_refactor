 ...
 !  print output from timestep nstep_print_lo to yaml%nstep_print
  ...
  logical,save :: hadtrop(1:8) = .false.
  ...
  ! below are levels / timeranges need to test tropopause_climate option for the above column
 ...
 !! if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
  ...
  !-----------------------------------------------------------------------------------------
     ...
     write(ext_str,'(A5,I1)') 'itrop',algorithm
     !-----------------------------------------------------------------------------------------
     ...
     if(.not.(hadtrop(algorithm))) then
        hadtrop(algorithm) = .true.
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('tropopause_findUsing', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'tropopause_findUsing',y_nstep, yaml%lev_print)

        ! add code for writing data here
        ! below are explicit input arguments to tropopause_findUsing
        ...
        call write_var(unit_input,unit_output,'algorithm',algorithm)
        ...
        ! below are external module inputs to tropopause_findUsing

        call write_var(unit_input,unit_output,'pcols',pcols)

        ! below are internal module inputs to tropopause_findUsing

        call write_var(unit_input,unit_output,'TROP_ALG_CLIMATE',TROP_ALG_CLIMATE)
