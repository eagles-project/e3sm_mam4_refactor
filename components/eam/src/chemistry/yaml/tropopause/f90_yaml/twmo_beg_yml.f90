...
!!  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('twmo', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'twmo',y_nstep, yaml%lev_print)

        ! add code for writing data here
        ! below are explicit input arguments to twmo
        ! Note:  output is function of all vertical levels, so all vertical levels of input printed
       
        call write_var(unit_input,unit_output,'temp1d',temp1d(:))
        call write_var(unit_input,unit_output,'pmid1d',pmid1d(:))
        call write_var(unit_input,unit_output,'plimu',plimu)
        call write_var(unit_input,unit_output,'pliml',pliml)
        call write_var(unit_input,unit_output,'gam',gam)
        call write_var(unit_input,unit_output,'trp',trp)

        ! below are internal module variable inputs to twmo

        call write_var(unit_input,unit_output,'cnst_kap',cnst_kap)
        call write_var(unit_input,unit_output,'cnst_faktor',cnst_faktor)

        ! below are internal module variable inputs to a dependent subroutine

        call write_var(unit_input,unit_output,'cnst_ka1',cnst_ka1)
