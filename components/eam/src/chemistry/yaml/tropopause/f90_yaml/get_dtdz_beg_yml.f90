...
!  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
  ...
  !  within vertical level and column loops, so use below condition to output only at specified combination

  if(yaml%col_print == y_i .and. yaml%lev_print == y_k .and. y_nstep<=yaml%nstep_print .and. y_nstep>=nstep_print_lo) then
  !-----------------------------------------------------------------------------------------
  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('get_dtdz', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'get_dtdz',y_nstep, yaml%lev_print)

        ! add code for writing data here
        ! below are explicit input arguments to get_dtdz
        call write_var(unit_input,unit_output,'pm',pm)
        call write_var(unit_input,unit_output,'pmk',pmk)
        call write_var(unit_input,unit_output,'pmid1d_up',pmid1d_up)
        call write_var(unit_input,unit_output,'pmid1d_down',pmid1d_down)
        call write_var(unit_input,unit_output,'temp1d_up',temp1d_up)
        call write_var(unit_input,unit_output,'temp1d_down',temp1d_down)

        ! below are internal module inputs to get_dtdz
        call write_var(unit_input,unit_output,'cnst_kap',cnst_kap)
        call write_var(unit_input,unit_output,'cnst_faktor',cnst_faktor)
