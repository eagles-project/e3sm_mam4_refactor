  ...
  !-----------------------------------------------------------------------------------------
     ...
     ! Example:"flag" in the code can be 0, 1, or 2, we can update "ext_str" as:
      write(ext_str,'(I2)') spc_inv_ndx
      ext_str = 'spc_inv_ndx_'//adjustl(ext_str)
     !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('calc_col_delta', &  !intent-in
  !           unit_input, unit_output) !intent-out
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'calc_col_delta',yaml%nstep_print, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'spc_is_inv',spc_is_inv)
        call write_var(unit_input,unit_output,'spc_ndx',spc_ndx)
        call write_var(unit_input,unit_output,'spc_inv_ndx',spc_inv_ndx)
        call write_var(unit_input,unit_output,'spc_exo_col',spc_exo_col(yaml%col_print))
        call write_var(unit_input,unit_output,'vmr',vmr(yaml%col_print,:,:))
        call write_var(unit_input,unit_output,'invariants',invariants(yaml%col_print,:,:))
