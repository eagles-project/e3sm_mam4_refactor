...
!  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('calc_exo_col', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'calc_exo_col',yaml%nstep_print, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'ncol',ncol)
        call write_var(unit_input,unit_output,'exo_coldens',exo_coldens(:,yaml%col_print,:))
        ! module variables
        call write_var(unit_input,unit_output,'last',last)
        call write_var(unit_input,unit_output,'next',next)
        call write_var(unit_input,unit_output,'ki',ki)
        call write_var(unit_input,unit_output,'delp',delp)
