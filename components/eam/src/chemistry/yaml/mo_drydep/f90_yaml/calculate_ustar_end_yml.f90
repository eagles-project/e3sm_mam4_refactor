  ...
  if(yaml%flag_print) then ! if this column exists in lchnk
     ...
     !start writing data
        ...
        call write_var(unit_output,'ustar',ustar(yaml%col_print,:))
        call write_var(unit_output,'cvar',cvar(yaml%col_print,:))
        call write_var(unit_output,'bycp',bycp(yaml%col_print,:))
