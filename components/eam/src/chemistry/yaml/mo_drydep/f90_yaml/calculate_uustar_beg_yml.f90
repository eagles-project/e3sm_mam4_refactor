...
!     ext_str = "loc3_over_water"
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'calculate_uustar',yaml%nstep_print, yaml%lev_print)
        ...
        call write_var(unit_input,unit_output,'lcl_frc_landuse',lcl_frc_landuse(yaml%col_print,:))
        call write_var(unit_input,unit_output,'va',va(yaml%col_print))
        call write_var(unit_input,unit_output,'zl',zl(yaml%col_print))
