  ...
  if(yaml%flag_print) then ! if this column exists in lchnk
     ...
     !start writing data
        ...
        call write_var(unit_output,'cts',cts(yaml%col_print))
        call write_var(unit_output,'rgsx',rgsx(yaml%col_print,:,:))
        call write_var(unit_output,'rsmx',rsmx(yaml%col_print,:,:))
