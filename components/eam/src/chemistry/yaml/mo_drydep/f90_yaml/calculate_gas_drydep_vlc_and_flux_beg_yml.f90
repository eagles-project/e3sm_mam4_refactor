  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'calculate_gas_drydep_vlc_and_flux',yaml%nstep_print, yaml%lev_print)
        ...
        call write_var(unit_input,unit_output,'lcl_frc_landuse',lcl_frc_landuse)
        call write_var(unit_input,unit_output,'mmr',mmr(yaml%col_print,:,:))
        call write_var(unit_input,unit_output,'dep_ra',dep_ra(yaml%col_print,:))
        call write_var(unit_input,unit_output,'dep_rb',dep_rb(yaml%col_print,:))
        call write_var(unit_input,unit_output,'term',term(yaml%col_print))
        call write_var(unit_input,unit_output,'rsmx',rsmx(yaml%col_print,:,:))
        call write_var(unit_input,unit_output,'rlux',rlux(yaml%col_print,:,:))
        call write_var(unit_input,unit_output,'rclx',rclx(yaml%col_print,:,:))
        call write_var(unit_input,unit_output,'rgsx',rgsx(yaml%col_print,:,:))
        call write_var(unit_input,unit_output,'rdc',rdc(yaml%col_print))
        call write_var(unit_input,unit_output,'rac',rac)
