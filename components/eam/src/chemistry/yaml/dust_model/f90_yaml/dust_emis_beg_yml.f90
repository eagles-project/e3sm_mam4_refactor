  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('dust_emis', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'dust_emis',yaml%nstep_print, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'ncol',ncol)
        call write_var(unit_input,unit_output,'lchnk',lchnk)
        call write_var(unit_input,unit_output,'dust_flux_in',dust_flux_in(yaml%col_print,:))
        call write_var(unit_input,unit_output,'soil_erod_fact',soil_erod_fact)
        call write_var(unit_input,unit_output,'soil_erodibility',soil_erodibility(yaml%col_print,y_lchnk))
        call write_var(unit_input,unit_output,'dust_density',dust_density)
        call write_var(unit_input,unit_output,'pi',pi)
        call write_var(unit_input,unit_output,'soil_erod_threshold',soil_erod_threshold)       
        call write_var(unit_input,unit_output,'dust_dmt_vwr',dust_dmt_vwr)
        call write_var(unit_input,unit_output,'dust_indices',dust_indices)
        call write_var(unit_input,unit_output,'dust_emis_sclfctr',dust_emis_sclfctr)
