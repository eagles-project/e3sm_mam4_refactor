  ...
  if(yaml%flag_print) then ! if this column exists in lchnk
     ...
     !start writing data
        ...
        call write_var(unit_output,'wrk_wd_mm',wrk_wd_mm(yaml%col_print,:))
        call write_var(unit_output,'noy_wk',noy_wk(yaml%col_print))
        call write_var(unit_output,'sox_wk',sox_wk(yaml%col_print))
        call write_var(unit_output,'nhx_wk',nhx_wk(yaml%col_print))
