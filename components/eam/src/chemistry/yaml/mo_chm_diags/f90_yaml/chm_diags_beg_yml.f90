  ...
  real(r8) :: fldcw_nn(ncol,pver,pcnst)

do nn = 1,pcnst
    fldcw => qqcw_get_field(pbuf,nn,lchnk,errorhandle=.true.)
    if(associated(fldcw))   fldcw_nn(:,:,nn) = fldcw(:ncol,:)
enddo
y_lchnk=lchnk
...
!  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('chm_diags', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'chm_diags',yaml%nstep_print, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'ncol',ncol)
        call write_var(unit_input,unit_output,'id_o3',id_o3)
        call write_var(unit_input,unit_output,'sox_species',sox_species)
        call write_var(unit_input,unit_output,'aer_species',aer_species)
        call write_var(unit_input,unit_output,'vmr',vmr(yaml%col_print,:,:))
        call write_var(unit_input,unit_output,'mmr',mmr(yaml%col_print,:,:))
        call write_var(unit_input,unit_output,'depvel',depvel(yaml%col_print,:))
        call write_var(unit_input,unit_output,'depflx',depflx(yaml%col_print,:))
        call write_var(unit_input,unit_output,'mmr_tend',mmr_tend(yaml%col_print,:,:))
...
do nn = 1,pcnst
    ...
    if(associated(fldcw)) then
        write(varname,'(I2)') nn
        varname = 'fldcw_nn'//adjustl(varname)
        call write_var(unit_input,unit_output,varname,fldcw_nn(yaml%col_print,:,nn))
    endif
enddo
