  ...
  character(len=200) :: mm_str, isec_str
  !-----------------------------------------------------------------------------------------
...
!  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('extfrc_set', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'extfrc_set',yaml%nstep_print, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'ncol',ncol)
        call write_var(unit_input,unit_output,'zint',zint(yaml%col_print,:))
        do mm = 1,extfrc_cnt
           write(mm_str,'(I2)') mm
           ...
           do isec = 1,forcings(mm)%nsectors
                ...
                call write_var(unit_input,unit_output,'forcings'//trim(adjustl(mm_str))//'_fields'//trim(adjustl(isec_str))//'_data', &
                               forcings(mm)%fields(isec)%data(yaml%col_print,:,lchnk))
           enddo
        enddo
