 ...
 !  print output for timesteps between nstep_print_lo and yaml%nstep_print
  if(yaml%col_print == y_i .and. y_nstep<=yaml%nstep_print .and. y_nstep>=nstep_print_lo .and. y_k == yaml%lev_print) then
  !-----------------------------------------------------------------------------------------
  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('calc_het_rates', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'calc_het_rates',y_nstep, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'satf',satf)
        call write_var(unit_input,unit_output,'rain',rain)
        call write_var(unit_input,unit_output,'xhen',xhen)
        call write_var(unit_input,unit_output,'tmp_hetrates',tmp_hetrates)
        call write_var(unit_input,unit_output,'work1',work1)
        call write_var(unit_input,unit_output,'work2',work2)
