...
!  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('gas_washout', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'gas_washout',y_nstep, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'plev',plev)
        call write_var(unit_input,unit_output,'xkgm',xkgm)
        call write_var(unit_input,unit_output,'xliq_ik',xliq_ik)
        call write_var(unit_input,unit_output,'xhen_i',xhen_i)
        call write_var(unit_input,unit_output,'tfld_i',tfld_i)
        call write_var(unit_input,unit_output,'delz_i',delz_i)
        call write_var(unit_input,unit_output,'xgas',xgas)
