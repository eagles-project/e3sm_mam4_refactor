...
!  yaml%lev_print = 51 !level (**remove these if generating data for a dependent subroutines**)
  ...
  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
     ...
     if ((n_calls==0 .and. mam_prevap_resusp_optcc==130) .or. (n_calls==84 .and. mam_prevap_resusp_optcc==230)) then
!     if (n_calls==0) then
     !<
...
!  if(yaml%col_print >0 .and. y_nstep==yaml%nstep_print .and. do_print) then
     ...
     call open_files('wetdep_prevap', &  !intent-in
...
call write_var(unit_input, unit_output, 'pprdx', pprdx)
call write_var(unit_input, unit_output, 'srcx', srcx)
call write_var(unit_input, unit_output, 'arainx', arainx)
