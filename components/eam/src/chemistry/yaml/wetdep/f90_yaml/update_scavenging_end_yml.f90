#ifdef YAML_WETDEP
  ! YAML file output generation code- DO NOT PORT to C++
  if(yaml%flag_print) then ! if this column exists in lchnk

     !write output header
     call write_output_header(unit_output)

     !start writing data
     !<add code for writing data here>
     call write_output_var(unit_output, 'scavt_ik', scavt_ik)
     call write_output_var(unit_output, 'iscavt_ik', iscavt_ik)
     call write_output_var(unit_output, 'icscavt_ik', icscavt_ik)
     call write_output_var(unit_output, 'isscavt_ik', isscavt_ik)
     call write_output_var(unit_output, 'bcscavt_ik', bcscavt_ik)
     call write_output_var(unit_output, 'bsscavt_ik', bsscavt_ik)
     call write_output_var(unit_output, 'rcscavt_ik', rcscavt_ik)
     call write_output_var(unit_output, 'rsscavt_ik', rsscavt_ik)
     call write_output_var(unit_output, 'scavabs', scavabs)
     call write_output_var(unit_output, 'scavabc', scavabc)
     call write_output_var(unit_output, 'precabc', precabc)
     call write_output_var(unit_output, 'precabs', precabs)


     !call write_output_var(unit_output, fld_name, field, inp_out_str)  !write a single output variable
     !call write_1d_output_var(unit_output, fld_name, dim, field, inp_out_str) !writes 1D variables of any dimension in the output python module
     !call write_2d_output_var(unit_output, fld_name, dim1, dim2, field, inp_out_str) !writes 2D variables of any dimension in the output python module

     !writes aerosol mmr from state%q or q vector(cloud borne and interstitial) in the output python module
     !"aer_num_only" is .ture. if printing aerosol num only
     !call write_output_aerosol_mmr_from_stateq(unit_output, fld_name, field, aer_num_only, inp_out_str)

     !close the output file
     close(unit_output)
     call freeunit(unit_output)
  endif

! find n_calls. the first condition should be consistent with *_beg_yml.f90 file
!if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print  .and. y_k == yaml%lev_print) then
!       write(102,*) 'n_calls = ',n_calls,mam_prevap_resusp_optcc, &
!        scavt_ik,iscavt_ik,icscavt_ik,isscavt_ik,bcscavt_ik,bsscavt_ik,rcscavt_ik,rsscavt_ik,&
!        scavabs,scavabc,precabc,precabs 
!endif
#endif
