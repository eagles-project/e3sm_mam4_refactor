  ...
  integer, intent(in) :: y_i, y_k, y_lchnk, y_pt, y_type
  !-----------------------------------------------------------------------------------------
...
!  integer,save :: n_calls=0
  ...
  logical,save :: hadpttype(2:3,0:1) = .false.
  ...
  !-----------------------------------------------------------------------------------------
     ...
     write(ext_str,'(A3,I1,A6,I1)') 'ypt',y_pt,'_ytype',y_type
...
!     if (n_calls==1) then ! output at the first call only, modify this if condition (see below) if writing out other calls
     ...
     !We only want to print the first data occurrence corresponding to each type of 'ypt' and 'ytype' within a timestep.
     !If current timestep (y_nstep) is the same as the timestep during the last pass of this code segment
     !(y_nstep_old), then we do not print if we have already printed that kind of ypt,ytype.
     ...
     if(.not.(hadpttype(y_pt,y_type))) then
 
        hadpttype(y_pt,y_type) = .true.
        ...
        call open_files('get_partition_factors', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'get_partition_factors',yaml%nstep_print, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'qgcm_intrst',qgcm_intrst)
        call write_var(unit_input,unit_output,'qgcm_cldbrn',qgcm_cldbrn)
