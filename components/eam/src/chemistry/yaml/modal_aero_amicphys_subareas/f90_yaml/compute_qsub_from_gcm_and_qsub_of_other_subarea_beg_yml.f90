  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('compute_qsub_from_gcm_and_qsub_of_other_subarea', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'compute_qsub_from_gcm_and_qsub_of_other_subarea',yaml%nstep_print, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'lcompute',lcompute(:))
        call write_var(unit_input,unit_output,'f_a',f_a)
        call write_var(unit_input,unit_output,'f_b',f_b)
        call write_var(unit_input,unit_output,'qgcm',qgcm(:))
        call write_var(unit_input,unit_output,'qsub_a',qsub_a(:))
        call write_var(unit_input,unit_output,'qsub_b',qsub_b(:))

        ! below are external module inputs to compute_qsub_from_gcm_and_qsub_of_other_subarea
