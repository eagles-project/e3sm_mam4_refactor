  ...
  if(yaml%flag_print) then ! if this column exists in lchnk
     ...
     !start writing data
        ...
        call write_var(unit_output,'qsub1',qsub1(:,:))
        call write_var(unit_output,'qsub2',qsub2(:,:))
        call write_var(unit_output,'qqcwsub2',qqcwsub2(:,:))
        call write_var(unit_output,'qsub3',qsub3(:,:))
        call write_var(unit_output,'qqcwsub3',qqcwsub3(:,:))
