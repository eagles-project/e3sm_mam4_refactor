  ...
  integer, intent(in) :: y_i, y_k, y_lchnk, y_pt
  !-----------------------------------------------------------------------------------------
...
!     if (n_calls==1) then ! output at the first call only, modify this if condition (see below) if writing out other calls
   ...
   !We only want to print the first data occurrence corresponding to each type of 'ypt' within a timestep.
     !If current timestep (y_nstep) is the same as the timestep during the last pass of this code segment
     !(y_nstep_old), then we do not print if we have already printed that kind of ypt.
     ...
     if(.not.(hadpt(y_pt))) then
        ...
        call open_files('set_subarea_qnumb_for_intrst_aerosols', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'set_subarea_qnumb_for_intrst_aerosols',yaml%nstep_print, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'loffset',loffset)
        call write_var(unit_input,unit_output,'jclea',jclea)
        call write_var(unit_input,unit_output,'jcldy',jcldy)
        call write_var(unit_input,unit_output,'fclea',fclea)
        call write_var(unit_input,unit_output,'fcldy',fcldy)
        call write_var(unit_input,unit_output,'qgcm',qgcm(:))
        call write_var(unit_input,unit_output,'qqcwgcm',qqcwgcm(:))
        call write_var(unit_input,unit_output,'qgcmx',qgcmx(:))
        call write_var(unit_input,unit_output,'qsubx',qsubx(:,:))

        ! below are external module inputs to set_subarea_qnumb_for_intrst_aerosols
