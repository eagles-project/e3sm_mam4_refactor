  ...
  if(yaml%flag_print) then ! if this column exists in lchnk
     ...
     !start writing data
        ...
        call write_var(unit_output,'newnuc_method_flagaa2',newnuc_method_flagaa2)
        call write_var(unit_output,'ratenucl',ratenucl)
        call write_var(unit_output,'rateloge',rateloge)
        call write_var(unit_output,'cnum_tot',cnum_tot)
        call write_var(unit_output,'cnum_h2so4',cnum_h2so4)
        call write_var(unit_output,'cnum_nh3',cnum_nh3)
        call write_var(unit_output,'radius_cluster',radius_cluster)
