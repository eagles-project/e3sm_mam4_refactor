  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('pbl_nuc_wang2008', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'pbl_nuc_wang2008',yaml%nstep_print, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'so4vol',so4vol)
        call write_var(unit_input,unit_output,'newnuc_method_flagaa',newnuc_method_flagaa)
        call write_var(unit_input,unit_output,'newnuc_method_flagaa2',newnuc_method_flagaa2)
        call write_var(unit_input,unit_output,'ratenucl',ratenucl)
        call write_var(unit_input,unit_output,'rateloge',rateloge)
        call write_var(unit_input,unit_output,'cnum_tot',cnum_tot)
        call write_var(unit_input,unit_output,'cnum_h2so4',cnum_h2so4)
        call write_var(unit_input,unit_output,'cnum_nh3',cnum_nh3)
        call write_var(unit_input,unit_output,'radius_cluster',radius_cluster)
