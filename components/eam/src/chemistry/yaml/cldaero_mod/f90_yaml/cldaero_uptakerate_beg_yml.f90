  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('cldaero_uptakerate', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'cldaero_uptakerate',yaml%nstep_print, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'xl',xl)
        call write_var(unit_input,unit_output,'cldnum',cldnum)
        call write_var(unit_input,unit_output,'cfact',cfact)
        call write_var(unit_input,unit_output,'cldfrc',cldfrc)
        call write_var(unit_input,unit_output,'tfld',tfld)
