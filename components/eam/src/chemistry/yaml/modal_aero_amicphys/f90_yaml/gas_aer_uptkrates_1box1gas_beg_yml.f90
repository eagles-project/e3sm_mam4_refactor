  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('gas_aer_uptkrates_1box1gas', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'gas_aer_uptkrates_1box1gas',yaml%nstep_print, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'accom',accom)
        call write_var(unit_input,unit_output,'gasdiffus',gasdiffus)
        call write_var(unit_input,unit_output,'gasfreepath',gasfreepath)
        call write_var(unit_input,unit_output,'beta_inp',beta_inp)
        call write_var(unit_input,unit_output,'n_mode',n_mode)
        call write_var(unit_input,unit_output,'dgncur_awet',dgncur_awet)
        call write_var(unit_input,unit_output,'lnsg',lnsg)
