  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('mam_soaexch_1subarea', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'mam_soaexch_1subarea',yaml%nstep_print, yaml%lev_print)
        ...
        call write_var(unit_input,unit_output,'lptr2_soa_a_amode',lptr2_soa_a_amode)
        ...
        call write_var(unit_input,unit_output,'npca',npca)
        call write_var(unit_input,unit_output,'npoa',npoa)
        call write_var(unit_input,unit_output,'nsoa',nsoa)
        call write_var(unit_input,unit_output,'nufi',nufi)
        call write_var(unit_input,unit_output,'ntot_amode',ntot_amode)
        call write_var(unit_input,unit_output,'iaer_pom',iaer_pom)
        call write_var(unit_input,unit_output,'mode_aging_optaa',mode_aging_optaa)

        call write_var(unit_input,unit_output,'a_min1',a_min1)
        call write_var(unit_input,unit_output,'g_min1',g_min1)
        call write_var(unit_input,unit_output,'alpha_astem',alpha_astem)
        call write_var(unit_input,unit_output,'dtsub_fixed',dtsub_fixed)
