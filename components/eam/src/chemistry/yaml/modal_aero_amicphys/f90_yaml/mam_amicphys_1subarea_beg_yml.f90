...
!!!  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        ! below is for a derived type input to mam_amicphys_1subarea.
        ! individual fields in the dervied type are written out to the data files.
        call write_var(unit_input,unit_output,'misc_vars_aa_sub%ncluster_tend_nnuc_1grid',misc_vars_aa_sub%ncluster_tend_nnuc_1grid)
#if ( defined ( MOSAIC_SPECIES ) )
        call write_var(unit_input,unit_output,'misc_vars_aa_sub%cnvrg_fail_1grid',misc_vars_aa_sub%cnvrg_fail_1grid)
        call write_var(unit_input,unit_output,'misc_vars_aa_sub%max_kelvin_iter_1grid',misc_vars_aa_sub%max_kelvin_iter_1grid)
        call write_var(unit_input,unit_output,'misc_vars_aa_sub%xnerr_astem_negative_1grid',misc_vars_aa_sub%xnerr_astem_negative_1grid(:,:))
#endif
        ...
        call write_var(unit_input,unit_output,'nair',nait)
