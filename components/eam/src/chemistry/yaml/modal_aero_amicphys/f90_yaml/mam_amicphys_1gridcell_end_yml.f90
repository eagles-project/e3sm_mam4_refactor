  ...
  if(yaml%flag_print) then ! if this column exists in lchnk
     ...
     !start writing data
        ...
        ! below is for a derived type output from mam_amicphys_1gridcell.
        ! individual fields in the dervied type are written out to the data files.
        call write_var(unit_output,'misc_vars_aa%ncluster_tend_nnuc_1grid',misc_vars_aa%ncluster_tend_nnuc_1grid)
#if ( defined ( MOSAIC_SPECIES ) )
        call write_var(unit_output,'misc_vars_aa%cnvrg_fail_1grid',misc_vars_aa%cnvrg_fail_1grid)
        call write_var(unit_output,'misc_vars_aa%max_kelvin_iter_1grid',misc_vars_aa%max_kelvin_iter_1grid)
        call write_var(unit_output,'misc_vars_aa%xnerr_astem_negative_1grid',misc_vars_aa%xnerr_astem_negative_1grid(:,:))
#endif
