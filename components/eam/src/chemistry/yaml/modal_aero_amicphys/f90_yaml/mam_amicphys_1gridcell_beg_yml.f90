...
!!!!  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
  if(yaml%col_print == i .and. y_nstep==yaml%nstep_print .and. k == yaml%lev_print) then ! if this column exists in y_lchnk
     ...
     write(103,*)  'i, k, nstep:  ',i,k,y_nstep
  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        ! below is for a derived type input to mam_amicphys_1gridcell.
        ! individual fields in the dervied type are written out to the data files.
        call write_var(unit_input,unit_output,'misc_vars_aa%ncluster_tend_nnuc_1grid',misc_vars_aa%ncluster_tend_nnuc_1grid)
#if ( defined ( MOSAIC_SPECIES ) ) 
        call write_var(unit_input,unit_output,'misc_vars_aa%cnvrg_fail_1grid',misc_vars_aa%cnvrg_fail_1grid)
        call write_var(unit_input,unit_output,'misc_vars_aa%max_kelvin_iter_1grid',misc_vars_aa%max_kelvin_iter_1grid)
        call write_var(unit_input,unit_output,'misc_vars_aa%xnerr_astem_negative_1grid',misc_vars_aa%xnerr_astem_negative_1grid(:,:))
#endif        

        ! below are external module inputs to mam_amicphys_1gridcell
        ...
        call write_var(unit_input,unit_output,'fcvt_gas',fcvt_gas(:))
        call write_var(unit_input,unit_output,'fcvt_num',fcvt_num)
        call write_var(unit_input,unit_output,'fcvt_gas',fcvt_aer(:))
        call write_var(unit_input,unit_output,'fcvt_wtr',fcvt_wtr)
        call write_var(unit_input,unit_output,'ngas',ngas)
        call write_var(unit_input,unit_output,'naer',naer)
        call write_var(unit_input,unit_output,'lmap_gas',lmap_gas(:))
        call write_var(unit_input,unit_output,'lmap_aer',lmap_aer(:,:))
        call write_var(unit_input,unit_output,'lmap_num',lmap_num(:))
        call write_var(unit_input,unit_output,'lmap_numcw',lmap_numcw(:))
        call write_var(unit_input,unit_output,'lmap_aercw',lmap_aercw(:,:))
