...
!  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
     ...
     ! if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then ! this is an example of writing multiple sets of YAML files
        ...
        call open_files('modal_aero_wateruptake_wetaer', &  !intent-in
...
!             unit_input, unit_output, trim(ext_str)) !intent-out, with the use of ext_str
        ...
        call write_var(unit_input, unit_output, 'ncol',ncol)!write a single variable
        call write_1d_var(unit_input, unit_output, 'rhcrystal', nmodes,rhcrystal) ! writes 1D variables of any dimension
        call write_1d_var(unit_input, unit_output, 'rhdeliques', nmodes,rhdeliques)
        call write_1d_var(unit_input, unit_output, 'dgncur_a', nmodes,dgncur_a(yaml%col_print, yaml%lev_print, :))
        call write_1d_var(unit_input, unit_output, 'dryrad', nmodes,dryrad(yaml%col_print, yaml%lev_print, :))
        call write_1d_var(unit_input, unit_output, 'hygro', nmodes,hygro(yaml%col_print, yaml%lev_print, :))
        call write_var(unit_input, unit_output, 'rh', rh(yaml%col_print,yaml%lev_print))
        call write_1d_var(unit_input, unit_output, 'naer', nmodes,naer(yaml%col_print, yaml%lev_print, :))
        call write_1d_var(unit_input, unit_output, 'dryvol', nmodes,dryvol(yaml%col_print, yaml%lev_print, :))
