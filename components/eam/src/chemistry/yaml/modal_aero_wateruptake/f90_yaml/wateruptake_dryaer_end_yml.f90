  ...
  if(yaml%flag_print) then ! if this column exists in lchnk
     ...
     !start writing data
     call write_1d_output_var(unit_output, 'hygro', nmodes, hygro(yaml%col_print, yaml%lev_print,:))
     call write_1d_output_var(unit_output, 'naer', nmodes, naer(yaml%col_print, yaml%lev_print,:))
     call write_1d_output_var(unit_output, 'dryrad', nmodes, dryrad(yaml%col_print, yaml%lev_print,:))
     call write_1d_output_var(unit_output, 'dryvol', nmodes, dryvol(yaml%col_print, yaml%lev_print,:))
     call write_1d_output_var(unit_output, 'drymass', nmodes, drymass(yaml%col_print, yaml%lev_print,:))
     call write_1d_output_var(unit_output, 'rhcrystal', nmodes,rhcrystal)
     call write_1d_output_var(unit_output, 'rhdeliques', nmodes, rhdeliques)
     call write_1d_output_var(unit_output, 'specdens_1', nmodes,specdens_1)
