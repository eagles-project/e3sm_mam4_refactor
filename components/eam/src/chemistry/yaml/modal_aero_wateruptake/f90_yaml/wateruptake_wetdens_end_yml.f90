  ...
  if(yaml%flag_print) then ! if this column exists in lchnk
     ...
     call write_1d_output_var(unit_output, 'wetdens',nmodes, wetdens(yaml%col_print,yaml%lev_print, :)) !writes 1D variables of any dimension in the output python module
...
!if(masterproc .and. y_nstep==yaml%nstep_print) then
!  write(102,*)'phys_debug_lat = ',get_lat(y_lchnk, yaml%col_print), &
!  ' phys_debug_lon = ', get_lon(y_lchnk, yaml%col_print), yaml%lev_print, wetdens(:, yaml%lev_print, 1), specdens_1(1)
!endif
