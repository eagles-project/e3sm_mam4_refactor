  ...
  if(yaml%flag_print) then ! if this column exists in lchnk
     ...
     call write_1d_output_var(unit_output, 'wetrad', nmodes, wetrad(yaml%col_print, yaml%lev_print,:)) !writes 1D variables of any dimension in the output python module
     call write_1d_output_var(unit_output, 'wetvol', nmodes,wetvol(yaml%col_print, yaml%lev_print,:))
     call write_1d_output_var(unit_output, 'wtrvol', nmodes,wtrvol(yaml%col_print, yaml%lev_print,:))
     call write_1d_output_var(unit_output, 'dgncur_awet', nmodes, dgncur_awet(yaml%col_print, yaml%lev_print,:))
     call write_1d_output_var(unit_output, 'qaerwat', nmodes, qaerwat(yaml%col_print, yaml%lev_print,:))
