  ...
  integer, intent(in) :: y_lchnk, icnst, itype, ifirst

  !  itype = 0 is interstitial 
  !        = 1 is cloud-borne
  !-----------------------------------------------------------------------------------------
...
!  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
  ...
  !-----------------------------------------------------------------------------------------
     ...
     if(itype == 0) then
        write(ext_str,'(A12,I2)') 'intstl_icnst',icnst
     elseif(itype == 1) then
        write(ext_str,'(A12,I2)') 'cldbrn_icnst',icnst
     endif
     !-----------------------------------------------------------------------------------------
     ...
     if(y_nstep == y_nstep_old .and. icnst == ifirst) then
        n_calls = n_calls+1
     else
        n_calls = 1
     endif
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('sedimentation_solver_for_1_tracer', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'sedimentation_solver_for_1_tracer',y_nstep, yaml%lev_print)

        ! add code for writing data here
        ! below are explicit input arguments to sedimentation_solver_for_1_tracer

        call write_var(unit_input,unit_output,'ncol',ncol)
        call write_var(unit_input,unit_output,'dt',dt)
        call write_var(unit_input,unit_output,'sed_vel',sed_vel(yaml%col_print,yaml%lev_print))
        call write_var(unit_input,unit_output,'qq_in',qq_in(yaml%col_print,yaml%lev_print))
        ...
        call write_var(unit_input,unit_output,'pint',pint(yaml%col_print,yaml%lev_print))
        ...
        ! below are external module inputs to sedimentation_solver_for_1_tracer
