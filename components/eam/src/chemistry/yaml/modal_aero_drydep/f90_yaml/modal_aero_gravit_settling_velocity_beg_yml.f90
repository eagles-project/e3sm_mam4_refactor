  ...
  !call:
  !call modal_aero_gravit_settling_velocity( imode,yaml, jvlc,moment, ncol, pcols, pver, radius_max,           &! in
  !subroutine:
  !subroutine modal_aero_gravit_settling_velocity( imode,yaml, jvlc,moment, ncol, pcols, nver, radius_max,           &! in
  ...
  if(yaml%flag_print) then
    ...
    if (imode == -999) then
     ...
     call open_files('modal_aero_gravit_settling_velocity', &  !intent-in
     ...
     call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
          'modal_aero_gravit_settling_velocity',yaml%nstep_print, yaml%lev_print)
     ...
     call write_var(unit_input,unit_output,'nver',nver)
