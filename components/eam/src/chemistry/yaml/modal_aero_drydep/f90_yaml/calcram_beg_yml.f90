 ...
 ! normally used with phys_debug_lat = 39.0553303768561, phys_debug_lon = 262.904774388088, but land point
 ! for water point use: phys_debug_lat = 38.2041160027685, phys_debug_lon = 197.020210451132
...
!  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('calcram', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'calcram',y_nstep, yaml%lev_print)
        ...
        call write_var(unit_input,unit_output,'ram1_in',ram1_in(yaml%col_print))
        ...
        call write_var(unit_input,unit_output,'fv_in',fv_in(yaml%col_print))
