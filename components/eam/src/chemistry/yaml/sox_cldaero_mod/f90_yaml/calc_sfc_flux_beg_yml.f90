  ...
  !-----------------------------------------------------------------------------------------
     ...
     if (n_calls==7) then ! output at the first call only, modify this if condition (see below) if writing out other calls
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('calc_sfc_flux', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'calc_sfc_flux',yaml%nstep_print, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'pdel',pdel(yaml%col_print,:))
        call write_var(unit_input,unit_output,'layer_tend',layer_tend(yaml%col_print,:))
