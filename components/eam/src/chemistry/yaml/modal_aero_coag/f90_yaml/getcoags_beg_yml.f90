  ...
  yaml%lev_print = 55 !level (**remove these if generating data for a dependent subroutines**)
  yaml%nstep_print = 1415 !time step(**remove these if generating data for a dependent subroutines**)
  ...
  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
     ...
     ! if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then ! this is an example of writing multiple sets of YAML files
        ...
        call open_files('getcoags', &  !intent-in
...
!             unit_input, unit_output, trim(ext_str)) !intent-out, with the use of ext_str
        ...
        call write_var(unit_input, unit_output,'lamda', lamda)
        call write_var(unit_input, unit_output,'kfmat', kfmat)
        call write_var(unit_input, unit_output,'kfmac', kfmac)
        call write_var(unit_input, unit_output,'kfmatac', kfmatac)
        call write_var(unit_input, unit_output,'knc', knc)

        call write_var(unit_input, unit_output,'dgatk', dgatk)
        call write_var(unit_input, unit_output,'dgacc', dgacc)
        call write_var(unit_input, unit_output,'sgatk', sgatk)
        call write_var(unit_input, unit_output,'sgacc', sgacc)
        call write_var(unit_input, unit_output,'xxlsgat', xxlsgat)
        call write_var(unit_input, unit_output,'xxlsgac', xxlsgac)
