  ...
  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
     ...
     ! if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then ! this is an example of writing multiple sets of YAML files
        ...
        call open_files('mam_coag_aer_update', &  !intent-in
...
!             unit_input, unit_output, trim(ext_str)) !intent-out, with the use of ext_str
        ...
        call write_1d_var(unit_input, unit_output, 'ybetaij3', max_coagpair, ybetaij3)
        call write_1d_var(unit_input, unit_output, 'qnum_tavg', max_mode, qnum_tavg)

        call write_2d_var(unit_input, unit_output, 'qaer_bgn', max_aer, max_mode, qaer_bgn)
        call write_2d_var(unit_input, unit_output, 'qaer_end', max_aer, max_mode, qaer_end)
