  ...
  if(yaml%flag_print) then ! if this column exists in lchnk
     ...
     !start writing data
        ...
        call write_var(unit_output,'tau',tau(yaml%col_print,0:pver,:))
        call write_var(unit_output,'tau_w',tau_w(yaml%col_print,0:pver,:))
        call write_var(unit_output,'tau_w_g',tau_w_g(yaml%col_print,0:pver,:))
        call write_var(unit_output,'tau_w_f',tau_w_f(yaml%col_print,0:pver,:))
