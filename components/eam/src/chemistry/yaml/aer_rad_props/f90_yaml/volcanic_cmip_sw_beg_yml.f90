  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('volcanic_cmip_sw', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'volcanic_cmip_sw',yaml%nstep_print, yaml%lev_print)
        ...
        call write_var(unit_input,unit_output,'ext_cmip6_sw_inv_m',ext_cmip6_sw_inv_m(yaml%col_print,:,:))
        call write_var(unit_input,unit_output,'ssa_cmip6_sw',ssa_cmip6_sw(yaml%col_print,:,:))
        call write_var(unit_input,unit_output,'af_cmip6_sw',af_cmip6_sw(yaml%col_print,:,:))
        call write_var(unit_input,unit_output,'tau',tau(yaml%col_print,0:pver,:))
        call write_var(unit_input,unit_output,'tau_w',tau_w(yaml%col_print,0:pver,:))
        call write_var(unit_input,unit_output,'tau_w_g',tau_w_g(yaml%col_print,0:pver,:))
        call write_var(unit_input,unit_output,'tau_w_f',tau_w_f(yaml%col_print,0:pver,:))
