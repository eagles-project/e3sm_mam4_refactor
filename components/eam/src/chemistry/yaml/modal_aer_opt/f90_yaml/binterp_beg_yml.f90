...
!  if(yaml%col_print>0 .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
...
!        yaml%flag_print  = .true.
        ...
        call open_files('binterp', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'binterp',yaml%nstep_print, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'ncoef',ncoef)
        call write_var(unit_input,unit_output,'prefr',prefr)
        call write_var(unit_input,unit_output,'prefi',prefi)
        call write_var(unit_input,unit_output,'table',reshape(table,(/size(table)/))) 
        call write_var(unit_input,unit_output,'ref_real',ref_real(yaml%col_print))
        call write_var(unit_input,unit_output,'ref_img',ref_img(yaml%col_print))
        call write_var(unit_input,unit_output,'ref_real_tab',ref_real_tab)
        call write_var(unit_input,unit_output,'ref_img_tab',ref_img_tab)
        call write_var(unit_input,unit_output,'itab',itab(yaml%col_print))
        call write_var(unit_input,unit_output,'jtab',jtab(yaml%col_print))
        call write_var(unit_input,unit_output,'ttab',ttab(yaml%col_print))
        call write_var(unit_input,unit_output,'utab',utab(yaml%col_print))
