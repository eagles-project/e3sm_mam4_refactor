  ...
  if(yaml%flag_print .and. n_calls==1) then ! if this column exists in lchnk
     ...
     !start writing data
        ...
        call write_var(unit_output,'itab',itab(yaml%col_print))
        call write_var(unit_output,'jtab',jtab(yaml%col_print))
        call write_var(unit_output,'ttab',ttab(yaml%col_print))
        call write_var(unit_output,'utab',utab(yaml%col_print))
        call write_var(unit_output,'coef',coef(yaml%col_print,:))
