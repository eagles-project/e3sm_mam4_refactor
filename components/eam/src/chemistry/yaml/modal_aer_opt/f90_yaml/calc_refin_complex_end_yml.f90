#ifdef YAML_OPT
  if(yaml%flag_print .and.  (n_calls==1 .or. n_calls==100)) then ! if this column exists in lchnk
     ...
     !start writing data
        ...
        call write_var(unit_output,'dryvol',dryvol(yaml%col_print))
        call write_var(unit_output,'wetvol',wetvol(yaml%col_print))
        call write_var(unit_output,'watervol',watervol(yaml%col_print))
        call write_var(unit_output,'refr',refr(yaml%col_print))
        call write_var(unit_output,'refi',refi(yaml%col_print))
...
!if(yaml%col_print>0 .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then
!      write(103,*) 'n_calls = ',n_calls, lwsw
!endif
!------------------------------------------------------------------------------------------------------------------------------
