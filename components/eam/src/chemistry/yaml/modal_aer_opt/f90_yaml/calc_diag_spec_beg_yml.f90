...
!  integer, intent(in) :: y_lchnk
  ...
  !-----------------------------------------------------------------------------------------
  !In the case of y_i or y_k are not passed as arguments, use the following if
  !condition:
!  if(yaml%col_print >0 .and. y_nstep==yaml%nstep_print) then
  !-----------------------------------------------------------------------------------------

  !-----------------------------------------------------------------------------------------
  !For generating data for a dependent subroutines where "yaml" derived type is
  !already
  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        !(**remove these yaml% variables if generating data for a dependent
        !subroutines**)
        ...
        call open_files('calc_diag_spec', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'calc_diag_spec',yaml%nstep_print, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'ncol',ncol)
        call write_var(unit_input,unit_output,'specmmr_k',specmmr_k(yaml%col_print))
        call write_var(unit_input,unit_output,'mass_k',mass_k(yaml%col_print))
        call write_var(unit_input,unit_output,'vol',vol(yaml%col_print))
        call write_var(unit_input,unit_output,'specrefr',specrefr)
        call write_var(unit_input,unit_output,'specrefi',specrefi)
        call write_var(unit_input,unit_output,'hygro_aer',hygro_aer)
