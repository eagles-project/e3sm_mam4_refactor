#ifdef YAML_OPT
  if(yaml%flag_print .and. (n_calls==1 .or. n_calls==5)) then ! if this column exists in lchnk
     ...
     !start writing data
        ...
        call write_var(unit_output,'radsurf',radsurf(yaml%col_print,:))
        call write_var(unit_output,'logradsurf',logradsurf(yaml%col_print,:))
        call write_var(unit_output,'cheb',cheb(:,yaml%col_print,:))
...
!------------------------------------------------------------------------------------------------------------------------------
!  if(yaml%flag_print) write(101,*) 'n_calls = ',n_calls, trim(ext_str)
#endif
