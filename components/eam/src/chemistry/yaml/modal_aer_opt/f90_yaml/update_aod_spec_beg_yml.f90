...
!  if(yaml%col_print >0 .and. y_nstep==yaml%nstep_print) then
  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('update_aod_spec', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'update_aod_spec',yaml%nstep_print, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'scath2o',scath2o)
        call write_var(unit_input,unit_output,'absh2o',absh2o)
        call write_var(unit_input,unit_output,'sumhygro',sumhygro)
        call write_var(unit_input,unit_output,'sumscat',sumscat)
        call write_var(unit_input,unit_output,'sumabs',sumabs)
        call write_var(unit_input,unit_output,'hygro_s',hygro_s)
        call write_var(unit_input,unit_output,'palb',palb)
        call write_var(unit_input,unit_output,'dopaer',dopaer)
        call write_var(unit_input,unit_output,'scat_s',scat_s)
        call write_var(unit_input,unit_output,'abs_s',abs_s)
        call write_var(unit_input,unit_output,'aod_s',aod_s)
