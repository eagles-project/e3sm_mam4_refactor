  ...
  if(yaml%flag_print) then ! if this column exists in lchnk
     ...
     !start writing data
        ...
        call write_var(unit_output,'nact',nact(:,:))
        call write_var(unit_output,'mact',mact(:,:))
        call write_var(unit_output,'qcld',qcld(:))
        call write_var(unit_output,'raercol_1',raercol(:,:,1))
        call write_var(unit_output,'raercol_cw_1',raercol_cw(:,:,1))
        call write_var(unit_output,'raercol_2',raercol(:,:,2))
        call write_var(unit_output,'raercol_cw_2',raercol_cw(:,:,2))
        call write_var(unit_output,'nnew',nnew)
        call write_var(unit_output,'nsav',nsav)
