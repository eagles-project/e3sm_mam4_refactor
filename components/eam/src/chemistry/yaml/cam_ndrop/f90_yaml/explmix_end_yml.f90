...
! note: since input condition was given for all levels, so is output condition
! since we are outside a vertical level loop, will print whole column by default below
     !start writing data
     ...
     call write_1d_output_var(unit_output, 'qnew', pver, qnew) !writes 1D variables of any dimension in the output python module
