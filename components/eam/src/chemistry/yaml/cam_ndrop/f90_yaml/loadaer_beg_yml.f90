  ...
  ! y_type     = 1 if called from within update_from_newcld, phase=1 (interstitial)
  !                state_q, cs derived from vertical level y_k
  !            = 2 if called from within update_from_cldn_profile, phase=1 (interstitial)
  !                state_q, cs are actually derived from vertical level y_k+1; 
  !            = 3 if called from ccncalc, phase=3 (interstitial+cloudborne)
  !            = 4 if called from within modal_aero_convproc, under chemistry (N/A for loadaer)
...
!        n_calls = n_calls+1
     ...
     if(.not.(hadtype(y_type))) then
        ...
        call open_files('loadaer', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'loadaer',y_nstep, yaml%lev_print)

        ! add code for writing data here
        ! below are explicit input arguments to loadaer
        
        call write_var(unit_input,unit_output,'state_q',state_q)
        call write_var(unit_input,unit_output,'cs',cs)
        call write_var(unit_input,unit_output,'phase',phase)

        ! below are optional input arguments to loadaer (should only be present if phase=3)
      
        if(present(qcldbrn1d)) then
           call write_var(unit_input,unit_output,'qcldbrn1d',qcldbrn1d)
        endif
        if(present(qcldbrn1d_num)) then
           call write_var(unit_input,unit_output,'qcldbrn1d_num',qcldbrn1d_num)
        endif

        ! below are external module variable inputs to loadaer

        call write_var(unit_input,unit_output,'maxd_aspectype',maxd_aspectype)

        ! below are ndrop module variable inputs to loadaer
