  ...
  if(yaml%flag_print) then ! if this column exists in lchnk
     ...
     !start writing data
        ...
        call write_var(unit_output,'fn',fn)
        call write_var(unit_output,'fm',fm)
        call write_var(unit_output,'fluxn',fluxn)
        call write_var(unit_output,'fluxm',fluxm)
        call write_var(unit_output,'flux_fullact',flux_fullact)
