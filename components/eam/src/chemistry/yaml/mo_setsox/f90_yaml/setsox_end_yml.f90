  ...
  if(yaml%flag_print) then ! if this column exists in lchnk
     ...
     !start writing data
        ...
        call write_var(unit_output,'qcw',qcw(yaml%col_print,yaml%lev_print,:))
        call write_var(unit_output,'qin',qin(yaml%col_print,yaml%lev_print,:))
...
!    do kk = 1,pver
!        write(102,*) kk, lwc(yaml%col_print,kk)
!    enddo
! endif
