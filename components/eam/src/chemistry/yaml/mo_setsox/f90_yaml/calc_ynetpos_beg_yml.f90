...
!  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
  ...
  !-----------------------------------------------------------------------------------------
     ...
     if (n_calls==4) then ! output at the first call only, modify this if condition (see below) if writing out other calls
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('calc_ynetpos', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'calc_ynetpos',yaml%nstep_print, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'yph',yph)
        call write_var(unit_input,unit_output,'fact1_so2',fact1_so2)
        call write_var(unit_input,unit_output,'fact2_so2',fact2_so2)
        call write_var(unit_input,unit_output,'fact3_so2',fact3_so2)
        call write_var(unit_input,unit_output,'fact4_so2',fact4_so2)
        call write_var(unit_input,unit_output,'Eco2',Eco2)
        call write_var(unit_input,unit_output,'Eh2o',Eh2o)
        call write_var(unit_input,unit_output,'Eso4',Eso4)
