  ...
  !(**remove yaml%lev_print, nstep_print, col_print if generating data for a
...
!  if(yaml%flag_print) then
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('calc_ph_values', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'calc_ph_values',yaml%nstep_print, yaml%lev_print)
        ...
        call write_var(unit_input,unit_output,'so4_fact',so4_fact)
        call write_var(unit_input,unit_output,'Ra',Ra)
        call write_var(unit_input,unit_output,'xkw',xkw)
