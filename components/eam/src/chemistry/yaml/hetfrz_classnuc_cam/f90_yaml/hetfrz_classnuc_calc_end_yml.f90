  ...
  !do icol = 1, ncol
  !   if (any(frzimm(icol,:)>0)) then
  !      write(103,*)''
  !      write(103,*)'BALLI:',get_lat(lchnk,icol),get_lon(lchnk,icol),get_nstep()
  !      write(103,*)count(frzimm(icol,:)>0.0)
  !   endif
  !enddo
  ...
  if(yaml%flag_print) then ! if this column exists in lchnk
     ...
     call write_var(unit_output, 'frzimm', pver, frzimm(icolp,:))
     call write_var(unit_output, 'frzcnt', pver, frzcnt(icolp,:))
     call write_var(unit_output, 'frzdep', pver, frzdep(icolp,:))


     !call write_output_var(unit_output, fld_name, field, inp_out_str)  !write a single output variable
     !call write_var(unit_output, '', dim, field, inp_out_str) !writes 1D variables of any dimension in the output python module
