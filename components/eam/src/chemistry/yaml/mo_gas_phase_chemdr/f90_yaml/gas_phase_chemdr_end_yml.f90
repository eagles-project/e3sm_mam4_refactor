  ...
  if(yaml%flag_print) then ! if this column exists in lchnk
     ...
     !start writing data
        ...
        call write_var(unit_output,'fsds',fsds(yaml%col_print))
        call write_var(unit_output,'ts',ts(yaml%col_print))
        call write_var(unit_output,'asdir',asdir(yaml%col_print))
        call write_var(unit_output,'precc',precc(yaml%col_print))
        call write_var(unit_output,'precl',precl(yaml%col_print))
        call write_var(unit_output,'snowhland',snowhland(yaml%col_print))
        call write_var(unit_output,'pblh',pblh(yaml%col_print))
        call write_var(unit_output,'drydepflx',drydepflx(yaml%col_print,:))
        call write_var(unit_output,'cflx',cflx(yaml%col_print,:))
        call write_var(unit_output,'qtend',qtend(yaml%col_print,:,:))
        call write_var(unit_output,'dgnum',dgnum(yaml%col_print,:,:))
        call write_var(unit_output,'dgnumwet',dgnumwet(yaml%col_print,:,:))
        call write_var(unit_output,'wetdens',wetdens(yaml%col_print,:,:))

!        call write_var(unit_output,'pbuf',pbuf)
