  ...
  !YAML file input generation code- DO NOT PORT to C++
     ...
     ! if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then ! this is an example of writing multiple sets of YAML files
        ...
        call open_files('initialize_dcondt', &  !intent-in
...
!             unit_input, unit_output, trim(ext_str)) !intent-out, with the use of ext_str
        ...
        call write_var(unit_input, unit_output, 'iflux_method', iflux_method)
        ...
        call write_1d_var(unit_input,unit_output,'fa_u',pver, fa_u)
        call write_1d_var(unit_input,unit_output,'mu_i', pverp,mu_i)
        call write_1d_var(unit_input,unit_output,'md_i', pverp,md_i)
        call write_2d_var(unit_input, unit_output,'chat', pcnst_extd,pverp,chat)
        call write_2d_var(unit_input, unit_output,'const', pcnst_extd, pver, const)
        call write_2d_var(unit_input, unit_output,'conu', pcnst_extd, pverp,conu)
        call write_2d_var(unit_input, unit_output,'cond', pcnst_extd, pverp,cond)
        call write_2d_var(unit_input, unit_output,'dconudt_activa', pcnst_extd,pverp,dconudt_activa)
        call write_2d_var(unit_input, unit_output,'dconudt_wetdep', pcnst_extd,pverp,dconudt_wetdep)
        call write_1d_var(unit_input,unit_output,'dudp', pver,dudp)
        call write_1d_var(unit_input,unit_output,'dddp', pver,dddp)
        call write_1d_var(unit_input,unit_output,'eudp', pver,eudp)
        call write_1d_var(unit_input,unit_output,'eddp', pver,eddp)
