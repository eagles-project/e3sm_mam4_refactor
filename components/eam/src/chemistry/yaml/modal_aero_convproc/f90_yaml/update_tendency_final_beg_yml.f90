  ...
  !print all inputs one-by-one at column "yaml%col_print"
     ...
     ! if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then ! this is an example of writing multiple sets of YAML files
        ...
        call open_files('update_tendency_final', &  !intent-in
...
!             unit_input, unit_output, trim(ext_str)) !intent-out, with the use of ext_str
        ...
        call write_var(unit_input, unit_output, 'kbot_prevap',kbot_prevap)
        call write_var(unit_input, unit_output, 'ntsub',ntsub)
        call write_var(unit_input, unit_output, 'jtsub',jtsub)
        ...
        call write_1d_var(unit_input, unit_output, 'sumactiva',pcnst_extd, sumactiva)
        call write_1d_var(unit_input, unit_output, 'sumaqchem',pcnst_extd, sumaqchem)
        call write_1d_var(unit_input, unit_output, 'sumwetdep',pcnst_extd, sumwetdep)
        call write_1d_var(unit_input, unit_output, 'sumresusp',pcnst_extd, sumresusp)
        call write_1d_var(unit_input, unit_output, 'sumprevap',pcnst_extd, sumprevap)
        call write_1d_var(unit_input, unit_output, 'sumprevap_hist',pcnst_extd, sumprevap_hist)
        call write_2d_var(unit_input, unit_output,'dqdt', pver,pcnst,dqdt(yaml%col_print, :,:))
        call write_2d_var(unit_input, unit_output, 'q_i', pver,pcnst, q_i)
