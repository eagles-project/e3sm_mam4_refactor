  ...
  !print all inputs one-by-one at column "yaml%col_print"
     ...
     ! if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then ! this is an example of writing multiple sets of YAML files
        ...
        call open_files('ma_activate_convproc', &  !intent-in
...
!             unit_input, unit_output, trim(ext_str)) !intent-out, with the use of ext_str
        ...
        call write_var(unit_input, unit_output, 'dt_u', dt_u)
        call write_var(unit_input, unit_output, 'wup', wup)
        ...
        call write_1d_var(unit_input,unit_output,'conu', pcnst_extd, conu)
        call write_1d_var(unit_input,unit_output,'dconudt', pcnst_extd, dconudt)
