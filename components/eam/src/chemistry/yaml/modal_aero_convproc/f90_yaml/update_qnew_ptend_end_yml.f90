  ...
  if(yaml%flag_print) then ! if this column exists in lchnk
     ...
     !start writing data
     call write_1d_output_var(unit_output, 'qnew', pcnst, qnew(yaml%col_print, yaml%lev_print,:))
     call write_1d_output_var(unit_output, 'ptend_lq',pcnst,ptend_lq)
     call write_1d_output_var(unit_output, 'ptend_q',pcnst,ptend_q(yaml%col_print, yaml%lev_print,:))
...
!      write(103,*) 'n_calls = ',n_calls, is_update_ptend
!endif
