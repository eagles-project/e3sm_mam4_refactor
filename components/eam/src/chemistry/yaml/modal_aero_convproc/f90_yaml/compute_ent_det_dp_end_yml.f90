  ...
  if(yaml%flag_print .and. n_calls==1) then ! if this column exists in lchnk
     ...
     !start writing data
     call write_output_var(unit_output,'ntsub',ntsub)
     call write_1d_output_var(unit_output,'eudp', pver,eudp(:))
     call write_1d_output_var(unit_output,'dudp', pver,dudp(:))
     call write_1d_output_var(unit_output,'eddp', pver,eddp(:))
     call write_1d_output_var(unit_output,'dddp', pver,dddp(:))
