  ...
  !print all inputs one-by-one at column "yaml%col_print"
     ...
     ! if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then ! this is an example of writing multiple sets of YAML files
        ...
        call open_files('initialize_tmr_array', &  !intent-in
...
!             unit_input, unit_output, trim(ext_str)) !intent-out, with the use of ext_str
        ...
        call write_var(unit_input, unit_output, 'pcnst_extd', pcnst_extd)!write a single variable
        call write_var(unit_input, unit_output, 'ncnst', ncnst)
        call write_var(unit_input, unit_output, 'iconvtype', iconvtype)
        call write_1d_var(unit_input, unit_output, 'doconvproc_extd', pcnst_extd, doconvproc_extd) ! writes 1D variables of any dimension
        call write_2d_var(unit_input, unit_output, 'q_i', pver,pcnst, q_i) ! writes 2D variables of any dimension: field(dim1,dim2)
