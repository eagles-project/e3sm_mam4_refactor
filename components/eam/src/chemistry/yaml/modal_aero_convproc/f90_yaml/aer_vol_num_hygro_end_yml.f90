  ...
  if(yaml%flag_print .and. yaml%lev_print==kk .and. n_calls==1) then ! if this column exists in lchnk
     ...
     call write_1d_output_var(unit_output, 'vaerosol', ntot_amode, vaerosol) !writes 1D variables of any dimension in the output python module
     call write_1d_output_var(unit_output, 'naerosol', ntot_amode, naerosol)
     call write_1d_output_var(unit_output, 'hygro', ntot_amode, hygro)
