  ...
  if(yaml%flag_print .and. n_calls==1) then ! if this column exists in lchnk
     ...
     !<add code for writing data here>
        call write_1d_output_var(unit_output, 'doconvproc',ncnst,doconvproc)
        call write_1d_output_var(unit_output, 'sumactiva',pcnst_extd, sumactiva)
        call write_1d_output_var(unit_output, 'sumaqchem',pcnst_extd, sumaqchem)
        call write_1d_output_var(unit_output, 'sumwetdep',pcnst_extd, sumwetdep)
        call write_1d_output_var(unit_output, 'sumresusp',pcnst_extd, sumresusp)
        call write_1d_output_var(unit_output, 'sumprevap',pcnst_extd, sumprevap)
        call write_1d_output_var(unit_output, 'sumprevap_hist',pcnst_extd, sumprevap_hist)
        call write_2d_output_var(unit_output,'dqdt', pver,pcnst,dqdt(yaml%col_print, :,:))
        call write_2d_output_var(unit_output, 'q_i', pver,pcnst, q_i)
        call write_2d_output_var(unit_output,'qsrflx', pcnst, nsrflx, qsrflx(yaml%col_print, :,:))
