  ...
  !print all inputs one-by-one at column "yaml%col_print"
     ...
     ! if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then ! this is an example of writing multiple sets of YAML files
        ...
        call open_files('compute_updraft_mixing_ratio', &  !intent-in
...
!             unit_input, unit_output, trim(ext_str)) !intent-out, with the use of ext_str
        ...
        call write_var(unit_input, unit_output,'iconvtype',iconvtype)
        call write_var(unit_input, unit_output,'dt',dt)
        call write_1d_var(unit_input,unit_output,'dp_i', pver,dp_i(:))
        ...
        call write_1d_var(unit_input,unit_output,'zmagl', pver,zmagl)
        call write_var(unit_input, unit_output,'dz',dz)
        ...
        call write_1d_var(unit_input,unit_output,'aqfrac', pcnst_extd, aqfrac)
