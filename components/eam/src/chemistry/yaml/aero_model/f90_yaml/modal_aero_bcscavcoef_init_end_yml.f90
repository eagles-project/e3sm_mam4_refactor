  ...
  if(yaml%flag_print) then ! if this column exists in lchnk
     ...
     call write_2d_output_var(unit_output, 'scavimptblnum', size(scavimptblnum,1), ntot_amode, scavimptblnum) !writes 2D variables of any dimension in the output python module
     call write_2d_output_var(unit_output, 'scavimptblvol', size(scavimptblvol,1), ntot_amode, scavimptblvol)
...
!  write(102,*) get_nstep(), shape(scavimptblnum),icolprnt(1) !,get_lat(lchunk, 1) 
!endif
