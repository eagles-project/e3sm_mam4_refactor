...
! if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('aero_model_gasaerexch', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'aero_model_gasaerexch',yaml%nstep_print, yaml%lev_print)
        ...
        call write_var(unit_input,unit_output,'mbar',mbar(yaml%col_print,:))
        call write_var(unit_input,unit_output,'zm',zm(yaml%col_print,:))
        call write_var(unit_input,unit_output,'qh2o',qh2o(yaml%col_print,:))
        call write_var(unit_input,unit_output,'cwat',cwat(yaml%col_print,:))
        call write_var(unit_input,unit_output,'cldfr',cldfr(yaml%col_print,:))
        call write_var(unit_input,unit_output,'cldnum',cldnum(yaml%col_print,:))
        call write_var(unit_input,unit_output,'airdens',airdens(yaml%col_print,:))
        call write_var(unit_input,unit_output,'vmr0',vmr0(yaml%col_print,:,:))
        ...
        call write_var(unit_input,unit_output,'adv_mass',adv_mass)
