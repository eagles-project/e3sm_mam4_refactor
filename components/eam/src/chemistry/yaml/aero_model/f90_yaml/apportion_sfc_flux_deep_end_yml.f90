  ...
  if(yaml%flag_print) then ! if this column exists in lchnk
     ...
     !start writing data
        ...
        call write_var(unit_output,'sflxbcdp',sflxbcdp(yaml%col_print))
        call write_var(unit_output,'sflxecdp',sflxecdp(yaml%col_print))
...
!if(masterproc .and. y_nstep>300 .and. y_nstep<400) then
!     write(102,*)y_nstep,n_calls,sflxbc(yaml%col_print)/sflxbcdp(yaml%col_print), sflxec(yaml%col_print)/sflxecdp(yaml%col_print)
!endif
