...
!  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
 ...
 !    write(ext_str,'(I2)') mmtoo_prevap_resusp(mm)
!     ext_str = 'mmtoo_'//adjustl(ext_str)
     !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('calc_resusp_to_coarse', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'calc_resusp_to_coarse',yaml%nstep_print, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'ncol',ncol)
        call write_var(unit_input,unit_output,'mm',mm)
        call write_var(unit_input,unit_output,'mmtoo_prevap_resusp',mmtoo_prevap_resusp)
        call write_var(unit_input,unit_output,'update_dqdt',update_dqdt)
        call write_var(unit_input,unit_output,'rcscavt',rcscavt(yaml%col_print,yaml%lev_print))
        call write_var(unit_input,unit_output,'rsscavt',rsscavt(yaml%col_print,yaml%lev_print))
        call write_var(unit_input,unit_output,'dqdt_tmp',dqdt_tmp(yaml%col_print,yaml%lev_print))
        call write_var(unit_input,unit_output,'rtscavt_sv',rtscavt_sv(yaml%col_print,yaml%lev_print,:))


        !irites aerosol mmr from state%q or q vector (cloud borne and interstitial)
