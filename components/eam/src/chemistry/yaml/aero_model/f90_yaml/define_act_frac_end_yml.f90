  ...
  if(yaml%flag_print) then ! if this column exists in lchnk
     ...
     !start writing data
        ...
        call write_var(unit_output,'sol_facti',sol_facti)
        call write_var(unit_output,'sol_factic',sol_factic(yaml%col_print,yaml%lev_print))
        call write_var(unit_output,'sol_factb',sol_factb)
        call write_var(unit_output,'f_act_conv',f_act_conv(yaml%col_print,yaml%lev_print))
