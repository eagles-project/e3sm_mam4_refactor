...
!  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
  ...
  !-----------------------------------------------------------------------------------------
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('apportion_sfc_flux_deep', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'apportion_sfc_flux_deep',yaml%nstep_print, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'ncol',ncol)
        call write_var(unit_input,unit_output,'rprddpsum',rprddpsum(yaml%col_print))
        call write_var(unit_input,unit_output,'rprdshsum',rprdshsum(yaml%col_print))
        call write_var(unit_input,unit_output,'evapcdpsum',evapcdpsum(yaml%col_print))
        call write_var(unit_input,unit_output,'evapcshsum',evapcshsum(yaml%col_print))
        call write_var(unit_input,unit_output,'sflxbc',sflxbc(yaml%col_print))
        call write_var(unit_input,unit_output,'sflxec',sflxec(yaml%col_print))
