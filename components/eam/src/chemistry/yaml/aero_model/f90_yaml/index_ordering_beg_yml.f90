...
!  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print .and. y_k == yaml%lev_print) then ! if this column exists in y_lchnk
  ...
  !-----------------------------------------------------------------------------------------
     ...
     write(ext_str,'(I2)') lspec
     ext_str = 'lspec_'//adjustl(ext_str)
     !-----------------------------------------------------------------------------------------
     ...
     if (n_calls==7 .or. n_calls==8 .or. n_calls==9 .or. n_calls==10 .or. n_calls==29) then ! covers different if options
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('index_ordering', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'index_ordering',yaml%nstep_print, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'lspec',lspec)
        call write_var(unit_input,unit_output,'imode',imode)
        call write_var(unit_input,unit_output,'lphase',lphase)
        call write_var(unit_input,unit_output,'jaeronumb',jaeronumb)
        call write_var(unit_input,unit_output,'jaeromass',jaeromass)
        call write_var(unit_input,unit_output,'jaerowater',jaerowater)
