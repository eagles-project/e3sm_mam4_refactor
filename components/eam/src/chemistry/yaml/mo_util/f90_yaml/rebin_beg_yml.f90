  ...
  integer, intent(in), optional :: y_lchnk, y_i, f
  !-----------------------------------------------------------------------------------------
  ...
  if(present(y_lchnk)) then

  !populate YAML structure
  !(**remove yaml%lev_print, nstep_print, col_print if generating data for a dependent subroutines**)
  yaml%lev_print = 40       !level, but not actually used since whole column is written out
  ...
  if(yaml%col_print == y_i .and. y_nstep==yaml%nstep_print ) then ! if this column exists in y_lchnk
  ...
  !-----------------------------------------------------------------------------------------
     ...
     if (n_calls==5) then ! output at the first call only, modify this if condition (see below) if writing out other calls
     ...
     !if ((n_calls==1 .and. flag==0) .or. (n_calls==3 .and. flag==1) .or. (n_calls==5 .and. flag==2)) then
        ...
        call open_files('rebin', &  !intent-in
        ...
        call write_input_output_header(unit_input, unit_output,yaml%lchnk_print,yaml%col_print, &
             'rebin',yaml%nstep_print, yaml%lev_print)

        ! add code for writing data here
        
        call write_var(unit_input,unit_output,'nsrc',nsrc)
        call write_var(unit_input,unit_output,'ntrg',ntrg)
        call write_var(unit_input,unit_output,'src_x',src_x(:))
        call write_var(unit_input,unit_output,'trg_x',trg_x(:))
        call write_var(unit_input,unit_output,'src',src(:))
