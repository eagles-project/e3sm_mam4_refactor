...
OPTIONS
     ...
     -elm_interface_mode      CLM4.5 only. Turn on and specify ELM interface mode. [off|bgc|pflotran] default is off.
                                 bgc      = run native bgc via the interface (for test purpose)
                                 pflotran = run coupled ELM-PFLOTRAN via the interface (BGC/H/TH coupling upon pflotran_clm.in)
...
sub setup_cmdl_elm_interface {
  ...
  if ( $physv->as_long() == $physv->as_long("clm4_0") ) {     
    if ($val ne "off"){
	  fatal_error("** Cannot turn elm_interface_mode on with clm4_0 physics.\n" );
    }
  } else {
    if ($val ne "off"){
        ...
        if ($val eq "bgc") {
           message("Using elm_interface mode: use_clm_bgc =.true.");

           if ($nl_flags->{"bgc_mode"} eq "sp"){
             fatal_error("-elm_interface_mode option of 'bgc' can ONLY be used with clm4_5 with -bgc cn|bgc");
      ...
      if ( $nl->get_value("use_pflotran") eq ".true." ) {
        if ($nl->get_value("use_elm_interface") ne ".true." ) {
           my $errorinfo = "\nNamelist 'use_pflotran=.true.' contradicts the command line option '-elm_interface_mode off'\n".
      	                     "Please set CLM namelist 'use_pflotran' via -elm_interface_mode pflotran";
      ...
      if ( $nl->get_value("use_clm_bgc") eq ".true." ) {
        if ($nl->get_value("use_elm_interface") ne ".true." ) {
          my $errorinfo = "\nNamelist 'use_clm_bgc=.true.' contradicts the command line option '-elm_interface_mode off'\n".
      	               "Please set CLM namelist 'use_clm_bgc' via -elm_interface_mode bgc\n".
